...
(defun run-command-recipes-go ()
  ...
  (let ((dir (run-command-recipes-project-root))
        ;; gop means - GO-p: it is buffer is Go lang?
        (gop (run-command-recipes-go-p)))
    (when (or gop (file-exists-p (expand-file-name "go.mod" dir)))
      (append
       ;; recipes which use this FILE
       (and (buffer-file-name) gop
            (list
             (list
              :command-name "go-fmt"
       ...
       ;; Recipes which use this PROJECT
       (list
        (list
